...
impl<'alloc> UpdateWithMeta<'alloc> for TypeDecl<'alloc> {
    fn update_with_meta(
        ...
        match (&*meta.word.str, &meta.value) {
            ("name", &MyNested::Value(ref name)) => self.set_name(name, arenas),
            ("use_", &MyNested::Value(ref name)) => self.set_use(name, arenas),
            _ => return Err(NotUpdated),
        }
        Ok(())
    }
        ...
        for nested0_syn in &meta_list.nested {
            ...
            match word {
                "UnsafeRepr" => {
                    ...
                    match nested0.value {
                        MyNested::MyList(list) => {
                            for elem in list {
                                this.main_repr = this.main_repr.or(match &*elem.word.str {
                                ...
                                match this.main_repr {
                                    Some(MainRepr::Rust) => {}
                                    Some(MainRepr::C) | Some(MainRepr::Transparent) | None => {
                ...
                "Type" => {
                    ...
                    update_ident_and_metadata(&nested0.value, item, TypeOrCConstr::Type, arenas);
                }
                "ConstValue" => {
            ...
            for param in list_2 {
                ...
                item.update_with_meta(&param, arenas).unwrap_or_else(|_| {
                    let error_msg = match type_or_cconstr {
                        TypeOrCConstr::Type => attribute_errors::type_subattrs(),
