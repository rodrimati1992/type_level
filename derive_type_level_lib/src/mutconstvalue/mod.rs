...
use attribute_detection::mutconstvalue::{CCAttributes, TypeDeclVariant};
...
use syn::{self, DeriveInput, GenericParam, Ident};
...
pub fn derive_from_derive_input(mut ast: DeriveInput) -> TokenStream {
    ...
    let new_ident = &|s: String| &*arenas.idents.alloc(parse_syn::parse_ident(&s));
    ...
    let (const_param_ident, const_param_default) = attrs.const_param.unwrap_or_else(|| {
    ...
    let ref type_alias_ty_params = type_params
        .iter()
        .map(|x| {
            let ident = &x.ident;
    ...
    let ref truncated_type_params = type_param_idents
        .iter()
    ...
    */
    ...
    let extra_reprs = attrs
        .additional_repr_attrs
    ...
    let field_accessors = field_indices
        .iter()
        .cloned()
        .zip(field_tys_mentioning_const)
