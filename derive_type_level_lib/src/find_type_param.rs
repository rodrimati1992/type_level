    ...
    pub fn in_(&self,other:&syn::Type)->bool{
        ...
        impl<'ftp,'ast> Visit<'ast> for FindTypeParamVisitor<'ftp> {
            ...
            fn visit_type(&mut self, i: &'ast syn::Type){
                ...
                if !self.found_param { visit::visit_type(self,i); }
            }
            
            fn visit_path(&mut self, i: &'ast syn::Path){
                self.found_param = 
                    self.found_param || 
                    self.ftp.type_param_ident == &i.segments[0].ident;
        ...
        impl ContainsIdent for TokenStream{
            #[inline]
            fn contains_ident(self,ident:&syn::Ident)->bool{
                self.into_iter().position(|tt| tt.contains_ident(ident) ).is_some()
            }
        }
        ...
        let mut visitor=FindTypeParamVisitor{ ftp:self, found_param:false };
