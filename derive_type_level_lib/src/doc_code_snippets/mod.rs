...
fn code_blocks<'a>(lines:&[CodeLineType<'a>])->BTreeMap<&'a str,CodeBlock<'a>>{
    ...
    for (line_number,line_type) in lines.iter().enumerate() {
        match *line_type{
            CodeLineType::Marker{name,position:MarkerPosition::Start}=>{
                let prev_code_block=code_blocks.insert(name,CodeBlock{
                    name,
                    start:line_number,
                    end:lines.len()-1,
                });
                if let Some(prev_code_block)=prev_code_block{
                    panic!("\n\ncan't reuse code block named:'{}' on line {}\n\n",
                        prev_code_block.name,
                        line_number
                    );
                }
                code_block_names.insert(name);
            }
            CodeLineType::Marker{name,position:MarkerPosition::End}=>{
...
fn expand_template(module_name:&str,code:&str,template:&str)->String{
    ...
    for (line_number,line) in template.lines().enumerate() {
        ...
        if line.starts_with(START_OF_MARKER) {
            ...
            let minimum_indentation=lines.iter().filter_map(|line|{
                match line {
    ...
    for attr in ast.attrs {
        let meta_list= match attr.interpret_meta() {
            Some(Meta::List(meta_list))=> 
                if meta_list.ident=="doccode"{ meta_list }else{ continue },
            _ => continue,
        };
        ...
        for nested0 in meta_list.nested {
            match nested0 {
                NestedMeta::Meta(Meta::NameValue(
                    MetaNameValue{ident,lit:Lit::Str(str_),.. }
                ))=>{
