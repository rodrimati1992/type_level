...
use submod_visibility::{DocHiddenAttr, IsPublic, MyVisibility};
...
impl<'a> FieldAccessorInfo<'a> {
    pub(crate) fn doc_hidden_attr(&self, common_tokens: &'a CommonTokens) -> DocHiddenAttr<'a> {
        DocHiddenAttr::new(IsPublic(self.public_instances != 0), common_tokens)
    }
}
...
impl<'a> StructDeclarations<'a> {
    pub fn new(
        ...
        let ref alloc_ident = |ident: Ident| -> &'a Ident { arenas.idents.alloc(ident) };
        let ref alloc_str = |s| -> &'a str { arenas.strings.alloc(s) };
        let ref ident_from =
            |ident: &str| -> &'a Ident { alloc_ident(Ident::new(ident, name.span())) };
        ...
        let orig_gens_item_use = original_generics_iter()
            .map(GenParamIn::item_use)
            .piped(|gens| quote!{ #(#gens,)* })
            .piped(|x| arenas.tokenstream.alloc(x));

        let orig_gens_item_decl = original_generics_iter()
            .map(GenParamIn::item_decl)
            .piped(|gens| quote!{ #(#gens,)* })
            .piped(|x| arenas.tokenstream.alloc(x));

        let orig_gens_impl_header = original_generics_iter()
            .map(GenParamIn::impl_header)
            .piped(|gens| quote!{ #(#gens,)* })
            .piped(|x| arenas.tokenstream.alloc(x));

        let basename_ty = outer_attr_sett.renames.basename.unwrap_or(name);

        let type_marker_struct = outer_attr_sett
            .renames
            .const_type
            .clone()
            .unwrap_or_else(|| ident_from(&format!("{}Type", basename_ty)));

        let type_trait_docs: &'a str =
            format!("A trait equivalent of `{}`.", basename_ty).piped(|s| arenas.strings.alloc(s));
        ...
        for variant in &ds.variants {
            let inner_attr_sett = match ds.enum_or_struct {
                EnumOrStruct::Enum => Cow::Owned(TLAttributes::new(variant.attrs, arenas)),
                EnumOrStruct::Struct => Cow::Borrowed(outer_attr_sett),
            };
            ...
            let fields = variant
                ...
                .map(|v| {
                        ...
                        EnumOrStruct::Struct => {
                            ...
                            relative_priv = if field_vis_kind < vis_kind {
                                match priv_field_vis {
                                    Some(ref mut pfv) => if *pfv < field_vis_kind {
                                        *pfv = field_vis_kind;
                                    },
                    ...
                    let accessor_field_ident = match name_ident {
                        FieldName::Index(i) => get_tuple_field_ident(i, relative_priv),
                        FieldName::Named(ident) => FieldAccessor::Struct(ident),
                    };
                    ...
                    let pub_trait_getter =
                        field_attrs.pub_trait_getter || relative_priv == RP::Inherited;
            ...
            let name: &'a Ident = inner_attr_sett
                .renames
                .variant_type
                .unwrap_or_else(|| match ds.enum_or_struct {
            ...
            let uninitialized_ident = ident_from(&format!("{}_Uninit", basename_vari));

            let trait_ident = inner_attr_sett
                .renames
                .trait_
                .unwrap_or_else(|| ident_from(&format!("{}Trait", basename_vari)));
            let wr_trait_ident = inner_attr_sett
                .renames
                .wr_trait
                .unwrap_or_else(|| ident_from(&format!("{}WithRuntime", basename_vari)));
            let discriminant_ident = ident_from(&format!("{}_Discr", basename_vari));
            let variant_marker_ident = ident_from(&format!("{}_Variant", basename_vari));
...
impl<'alloc> StructDeclaration<'alloc> {
    pub(crate) fn is_item_declared(
        ...
        let decls_is_impld = declarations.attribute_settings.derived[item]
            .inner
            .is_implemented();
        if matches!(ImplIndex::IntoConstType | ImplIndex::IntoRuntime = item) {
            return decls_is_impld;
        }
        decls_is_impld || self.attribute_settings.derived[item].inner.is_implemented()
    }
}
...
impl<'a> ToTokens for StructDeclarations<'a> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        for declaration in &self.declarations {
            ...
            let additional_derives = additional_derives
                .is_empty()
            ...
            let field_vis = declaration
                .fields
                .iter()
                .map(|x| x.vis_kind.submodule_level(1));

            let variant_docs = format!("The ConstValue equivalent of {}", declaration.variant_str);
...
impl<'a> FieldDeclaration<'a> {
    pub(crate) fn doc_hidden_attr(&self) -> DocHiddenAttr {
        DocHiddenAttr::new(IsPublic(self.pub_trait_getter), self.common_tokens)
    }
}
...
impl<'a> FieldName<'a> {
    fn new(fi: &'a FieldIdent<'a>) -> Self {
        match fi {
            &FieldIdent::Index(index, ..) => FieldName::Index(index),
            &FieldIdent::Named(name) => FieldName::Named(name),
        }
    }
}
...
impl<'a> ToTokens for FieldName<'a> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        match *self {
            FieldName::Index(ind) => syn::Index::from(ind).to_tokens(tokens),
            FieldName::Named(name) => name.to_tokens(tokens),
        }
    }
}
