...
#[allow(unused_imports)]
use core_extensions::{BoolExt, OptionExt};
...
impl<'a> ToTokens for CompiletimeTraits<'a> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let supertrait = match self.decls.enum_or_struct {
            EnumOrStruct::Enum => quote!{ #enum_trait },
            EnumOrStruct::Struct => quote!{ __DerivedTraits<Type=#type_marker_struct> },
        };

        {
            let self_ty =
                ToTokenFnMut::new(
                    |tstream| match derived.into_consttype.inner.to_specified() {
                        ImplVariant::Unspecified(_) | ImplVariant::NoImpls => {}
                        ImplVariant::DefaultImpls => original_type.to_tokens(tstream),
            ...
            let tmp = match derived {
                ImplVariant::Unspecified(_) => false,
                ImplVariant::NoImpls => false,
                ImplVariant::DefaultImpls | ImplVariant::Internal { .. } => true,
            };
        ...
        for struct_ in &self.decls.declarations {
            ...
            let field_mod_fn = || {
                struct_.fields.iter().map(|x| match x.relative_priv {
            ...
            let field_name_vis_fn = || struct_.fields.iter().map(|f| f.doc_hidden_attr());
            ...
            let constructor =
                ToTokenFnMut::new(
                    |tstream| match (specified_ir_iv, self.decls.enum_or_struct) {
            ...
            let discriminant_ident = derived
                .get_discriminant
                .inner
                .is_implemented()
                    ...
                    for field in &struct_.fields {
                        ...
                        match c_or_r {
                            ConstOrRuntime::Const => for bound in bounds {
                                let removed_self =
                                    SelfRemovedBound::new(bound.clone(), |i: &Ident| {
                                        struct_.assoc_ty_to_generics.get(i).map(|x| (*x).clone())
                                    });
                                removed_self.to_tokens(t_s);
                                token.add.to_tokens(t_s);
                            },
                            ConstOrRuntime::Runtime => bounds.to_tokens(t_s),
                        }
            ...
            let const_bounds_opt_colon = struct_
                .fields
                .iter()
            ...
            let runt_bounds_opt_colon = struct_
                .fields
                .iter()
