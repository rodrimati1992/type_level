...
impl<T> ExactSizeIterator for DisabledEnabled<T> 
where T:Default
{}
...
fn test_enabled_disabled_iter(){
    ...
    assert_eq!(DisabledEnabled::new("what").collect::<Vec<_>>(), vec![ "","what" ]);
    assert_eq!(DisabledEnabled::new(true).collect::<Vec<_>>(), vec![ false,true ]);
}